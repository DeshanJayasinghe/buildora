# BuildWise — Frontend Engineering Standards & Reference Implementation

> **File name:** `BuildWise_Frontend_Engineering_Standards.md`  
> **Audience:** Codex, Claude, human contributors, reviewers  
> **Applies to:** `apps/web`, `packages/design-system`, `packages/cad-2d`, `packages/engine-3d`  
...
This document defines how BuildWise frontend code should be written so the product remains consistent while supporting very different interfaces:
...
React UI
  ...
  └── AI Panel
        ↓
Editor / 3D Controller
        ↓
PixiJS / Three.js Adapters
        ↓
Derived Render Models
        ↓
Building Model commands/API
```
...
| server project data | Server Component / TanStack Query |
| form fields | React Hook Form/local |
| open drawer/tab | local state/Zustand |
| selected editor tool | editor UI store |
| camera | 3D/Pixi engine |
| hover | engine/local |
...
export function CreateProjectForm() {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      {/* Use BuildWise design-system form controls. */}
    </form>
  );
}
```
...
→ domain command
→ API/backend
→ committed Building Model version
→ renderer refresh
```

PixiJS objects are disposable views.
...
```ts
import { Button } from "@buildwise/design-system";
```
...
> **the user experiences one coherent BuildWise product, while server state, form state, UI state, domain state and rendering-engine state remain clearly separated and every layer owns only what it should.**
