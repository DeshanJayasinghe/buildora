# Buildora AI — Codex Implementation Reference

> **Purpose:** Source-of-truth implementation guidance for Buildora AI. Use this file together with other project `.md` files added later.
>
...
Buildora AI is an **AI Building Design, Quantity Surveying, Costing, and Construction Intelligence Platform**.
...
Buildora AI must not behave like a simple AI image generator. The output must become structured, editable construction data.
...
- WebGPU deferred to post-beta progressive enhancement, only if profiling
  shows a ceiling it relieves
...
Select
 ↓
Create Buildora AI Building Model
```
...
This is a major Buildora AI differentiator from consumer-only 3D planning applications.
...
```text
Read Buildora_AI_Codex_Implementation_Reference.md as the current implementation reference.

Before writing code:
1. Identify the feature/module being implemented.
2. Identify the relevant architectural rules from this file.
3. Preserve the Buildora AI Building Model as the source of truth.
