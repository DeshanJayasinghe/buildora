# BuildWise — Backend Engineering Standards & Reference Implementation

> **File name:** `BuildWise_Backend_Engineering_Standards.md`  
> **Audience:** Codex, Claude, human contributors, reviewers  
> **Applies to:** `apps/api`, backend workers, and shared domain/backend packages  
> **Primary stack:** NestJS + TypeScript + PostgreSQL + Drizzle + Redis + Temporal + provider adapters  
...
This document defines how BuildWise backend code must be written so the codebase stays consistent while the SaaS grows from projects and tenancy into Building Model, QS, BOQ, cost, AI, BIM, documents, billing and enterprise workflows.
...
BuildWise API errors should use `application/problem+json`.

Example:

```json
{
  "type": "https://buildwise.example/problems/project-not-found",
...
export class ProblemDetailsFilter implements ExceptionFilter {
    ...
    if (exception instanceof AppError) {
      response
        ...
        .json({
          type: `https://buildwise.example/problems/${exception.code
            .toLowerCase()
            .replaceAll("_", "-")}`,
    ...
    response
      ...
      .type("application/problem+json")
      .json({
        type: "https://buildwise.example/problems/internal-error",
...
```json
{
  "type": "https://buildwise.example/problems/validation-failed",
...
import type { CreateProjectInput } from "@buildwise/api-contracts/projects";
...
Use idempotency for externally retried operations that must not duplicate effects.
...
```json
{
  "type": "https://buildwise.example/problems/model-version-conflict",
