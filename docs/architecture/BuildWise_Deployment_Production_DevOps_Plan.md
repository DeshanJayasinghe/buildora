# BuildWise — Deployment, Production & DevOps Architecture Plan

> **File name:** `BuildWise_Deployment_Production_DevOps_Plan.md`
>
> **Purpose:** Canonical deployment, production operations, CI/CD, infrastructure, security, observability, backup, disaster recovery, scaling, and DevOps plan for BuildWise.
>
...
>
> **Important:** BuildWise should remain deployable as ordinary containers. Do not introduce Kubernetes, service mesh, multi-region active/active, or unnecessary microservices before measured production requirements justify them.

---

# 1. Purpose

BuildWise is a production SaaS containing:
...
BuildWise follows these principles.
...
Do not deploy BuildWise to Kubernetes merely because it may eventually scale.
...
```text
app.buildwise.example
api.buildwise.example
```

Optional:

```text
status.buildwise.example
docs.buildwise.example
```
...
buildwise-api
buildwise-ai-worker
buildwise-bim-worker
buildwise-render-worker
```
...
```text
buildwise-api:9f4e1cb...
buildwise-api:release-1.2.0
```
...
RUN useradd --create-home buildwise

USER buildwise

CMD ["uv", "run", "python", "-m", "buildwise_ai.worker"]
```
...
Production workers run inside BuildWise compute environment.
...
```text
buildwise-ai-v1
buildwise-plan-processing-v1
buildwise-bim-v1
buildwise-reports-v1
buildwise-render-v1
```
...
Keep BuildWise on standard PostgreSQL semantics so migration to:
...
jobs:
  typescript:
    ...
    services:
      postgres:
        ...
        env:
          ...
          POSTGRES_PASSWORD: buildwise_test
        ports:
          - 5432:5432
        options: >-
          --health-cmd="pg_isready -U buildwise -d buildwise_test"
          --health-interval=5s
          --health-timeout=5s
          --health-retries=10
    ...
    env:
      DATABASE_URL: postgresql://buildwise:buildwise_test@localhost:5432/buildwise_test
...
Because BuildWise uses standard PostgreSQL, migration should remain feasible.
...
If BuildWise sells into regulated/enterprise markets:
...
docs/architecture/BuildWise_Deployment_Production_DevOps_Plan.md
...
Normal BuildWise releases should avoid downtime.
...
BuildWise DevOps must preserve these rules:
...
For BuildWise, the correct production philosophy is:
