# BuildWise — Billing, Subscription, Credits & AI Usage Architecture

> **File name:** `BuildWise_Billing_Architecture.md`
>
> **Purpose:** Production-grade billing architecture for BuildWise as a multi-tenant AI SaaS.
>
...
>
> **Primary implementation stack:** NestJS + TypeScript + PostgreSQL + Drizzle + Stripe + BuildWise AI Gateway + Temporal
>
...
BuildWise billing must solve more than recurring subscriptions.

BuildWise has variable costs from:
...
BuildWise can update its internal routing/rate card without immediately changing the customer's plan language.

---

# 3. Sources of Truth

BuildWise billing uses multiple authoritative systems for different responsibilities.
...
- BuildWise feature permissions,
- AI credit balances,
- project limits,
- AI token cost,
- BuildWise usage reservations,
- BuildWise gross-margin decisions.

---

## 3.2 BuildWise Billing Core

BuildWise is authoritative for:
...
The BuildWise AI Gateway is authoritative for captured provider usage such as:
...
The current OpenAI Responses API returns usage including input tokens, output tokens, total tokens, cached input details, cache-write details, and output reasoning-token details. BuildWise must persist the raw usage breakdown rather than only storing `total_tokens`.

---

## 3.4 Provider Cost Rate Cards

BuildWise stores a versioned internal rate card.
...
A BuildWise organization is the customer account.
...
Do not use the Stripe customer ID as the BuildWise primary key.
...
Because BuildWise may later change:
...
BuildWise creates an effective entitlement state from:
...
BuildWise should normalize external subscription status into an internal state.
...
This gives BuildWise a stable period for:
...
However BuildWise must still protect margin.
...
User chooses plan
    ↓
BuildWise creates checkout intent/session
    ↓
Stripe Checkout
    ↓
Stripe handles payment
    ↓
User returns to BuildWise
    ↓
BuildWise shows "Activating subscription…"
    ↓
...
Stripe can create prorations when subscription prices/items change. BuildWise should preview the proration before applying the upgrade.
...
BuildWise can display a preview returned from Stripe.

Keep BuildWise internal credit adjustments separate from currency proration.
...
If provider fails and BuildWise retries:
...
```text
charge only when BuildWise produces a usable result
```
...
The exact set depends on integration, but BuildWise should expect events around:
...
```text
buildwise:subscription-upgrade:{operationId}
```
...
BuildWise should not receive/store:
...
If plan catalog changes during the month, BuildWise can explain:
...
If BuildWise later uses third-party generative image APIs for visualization:
...
If BuildWise later charges customers postpaid for actual metered usage:
...
keep the BuildWise internal usage ledger.
...
BuildWise does **not** need to adopt provider-side usage metering for MVP because its initial model is primarily:

```text
subscription + included internal credits + top-ups
```

If BuildWise later introduces true postpaid overage billing, evaluate Stripe's current usage-based/Metronome capabilities rather than building invoice-grade metering from scratch.

The BuildWise internal ledger remains necessary regardless.
...
```json
{
  "type": "https://buildwise.example/problems/insufficient-ai-credits",
...
`402 Payment Required` may be used for commercial credit exhaustion if adopted consistently; alternatively BuildWise can use a documented `403` policy. Choose once and standardize.
...
BuildWise records operation result.
...
This is both good UX and aligned with the BuildWise architecture.
...
so BuildWise can later explain why:
...
BuildWise operational billing records are not automatically a full accounting ledger.
...
BuildWise may still need custom plan-change UI to coordinate:

- credits,
- entitlements,
- downgrade policy.

Do not allow a portal setting that bypasses BuildWise internal plan-change rules unless webhook/reconciliation fully supports it.
...
For first paid BuildWise release:
...
# 236. BuildWise Billing Lifecycle — Full View
...
BuildWise billing must always preserve these rules:

1. **Customer credits are not raw AI tokens.**
2. **Raw provider usage is still recorded precisely.**
3. **Stripe is payment authority; BuildWise is entitlement/usage authority.**
...
- Stripe currently recommends its Metronome-based usage platform for new integrations requiring advanced usage-based billing features such as prepaid credits, high-volume event ingestion, enterprise commitments, dimensional pricing, and real-time usage visibility. BuildWise's MVP does not need to depend on that because customer-facing Design/Render credits can be maintained internally while Stripe handles subscription/top-up payments.
- OpenAI's Responses API exposes structured usage including input tokens, output tokens, total tokens, cached-input details, cache-write details, and reasoning-token details. BuildWise should preserve these provider usage fields for internal cost and margin analysis.
...
The correct BuildWise billing model is:
...
This separation gives BuildWise:
