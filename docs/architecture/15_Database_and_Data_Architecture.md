# Buildora AI — Comprehensive Database & Data Architecture

> **File name:** `15_Database_and_Data_Architecture.md`
>
> **Purpose:** Canonical database and data architecture for the Buildora AI multi-tenant SaaS platform.
>
...
Buildora AI is not a conventional CRUD SaaS.
...
Buildora AI uses different storage systems for different types of data.
...
# 3.1 Contract Gates Affecting This Schema

Three implementation contracts must be published before the tables they govern
carry production data. See `OPEN_ARCHITECTURE_DECISIONS.md` §1.1.

| Gate | Constrains | Schema consequence |
|---|---|---|
| **CG-1** | Room topology determinism | `building_elements` (SPACE) boundary semantics, `element_relationships` `BOUNDS`, and **room identity across split/merge/delete** |
| **CG-2** | Session reconciliation | `model_commands` idempotency and the durable pending-queue contract |
| **CG-3** | QS rule contract | `qs_rule_sets` needs a **typed rule document plus a versioned primitive bundle**, both hashed — `rules_hash` alone is insufficient, because two engine versions could interpret one hash differently |

**CG-3 is the one with a direct schema implication.** `qs_rule_sets.rules_hash`
must hash the canonical rule document **and** the executable primitive/engine
bundle version, or historical quantity runs are not reproducible.
...
> **LOCKED — ADR-004.** This section is a frozen architecture decision.
> It may be changed only by an approved ADR that explicitly supersedes ADR-004.
...
`public_id` is **never a foreign-key target**. Relational integrity uses the `uuid` primary key only.

Uniqueness of `public_id` is enforced by a database unique index; generation alone is not sufficient. Retry on conflict.

> **Resolved (ADR-004, 2026-09-12).** Earlier CRUD examples used prefixed `varchar` primary keys. That pattern is **withdrawn**. `docs/examples/Buildora_AI_Sample_Backend_CRUD.md` has been corrected to `uuid` primary keys plus `public_id`. No document in this repository now shows a prefixed varchar primary key.
...
## Every user has an organization — always (ADR-021)

There is **no separate personal-ownership path**. A solo user gets a **personal
organization** created automatically at signup, of which they are sole owner.

```text
signup → user + personal organization (auto-created) → projects belong to that org
```
...
-------------
...
kind                 PERSONAL | TEAM        (ADR-021)
converted_at         nullable — when a PERSONAL org became TEAM
```

**Why this is binding.** ADR-016 makes organization the tenant boundary. A parallel
"projects owned directly by a user" path would require a second code path through
every tenant-scoped query, every RLS policy and every permission check — and the
second path is where a cross-tenant leak eventually appears.

Because ownership never changes shape:

```text
homeowner → property developer        rename + invite members
freelance architect → practice        rename + invite members
```

are a **rename and an invitation**, not a data migration. `kind` may flip
`PERSONAL → TEAM`; no project row moves.

The UI should not make a solo user think about organizations at all. The concept
exists in the schema, not necessarily in their experience.
...
draft
active
archived        ← paid term ended OR user archived. DATA PRESERVED.
deleting
deleted
```
...
## Archive is not deletion (ADR-021)

`archived` is the state a project enters when a **fixed-term plan expires** or a
user archives it deliberately. **The project's data is preserved in full.**

```text
draft → active → archived → deleting → deleted
                    ▲
     paid term ends HERE, never at `deleting`
```

Archive capability is expressed through **entitlements**, not by removing data:

| Archived project | Allowed |
|---|---|
| Read model, 2D, 3D, walkthrough | ✓ |
| Read previously generated reports and exports | ✓ |
| Read version history | ✓ |
| Export own data per policy | ✓ |
| **Model commands (edit)** | ✗ |
| **AI jobs** | ✗ |
| **New quantity/cost runs, new exports** | ✗ |

Reactivation restores full capability **against the existing model** — no data
was lost, so the user resumes rather than restarts.

Optional lifecycle detail columns:

```text
projects
--------
archived_at
archive_reason        TERM_EXPIRED | USER_ARCHIVED | PLAN_DOWNGRADE
reactivated_at
```

Artifacts belonging to archived projects are candidates for **cold storage
lifecycle rules** (object storage class transition), never for deletion while the
project is merely archived.
...
name
sequence
elevation_mm                    AUTHORITATIVE vertical fact (ADR-005)
default_storey_height_mm        NULLABLE. Design aid only.
...
> **LOCKED — ADR-005.** `elevation_mm` is the **single authoritative vertical
> fact** for a level. Floor-to-floor is **derived**
> (`next.elevation_mm − this.elevation_mm`) and must never be stored as an
> independent column.
>
> `default_storey_height_mm` is a creation-time convenience used when defaulting
> new element heights. It must **never** be read by elevation, floor-to-floor,
> stair-rise, volume or export calculations. A schema invariant test enforces this.
>
> *(Corrected 2026-09-12: this table previously carried a generic `height_mm`,
> which an implementer could not distinguish from an authoritative storey height.
> Independent review found it contradicted ADR-005.)*
...
element_type                  WALL | DOOR | WINDOW | ROOM | SLAB |
                              COLUMN | BEAM | STAIR | ROOF |
                              OPENING | FURNITURE | FIXTURE | EQUIPMENT
...
assembly_version_id nullable  construction cost expansion   (ADR-007)
ffe_asset_version_id nullable FF&E catalogue product        (ADR-019)
...
# 36.1 Element Class — Why It Exists (ADR-019)

`element_class` governs **which subsystems see an element**. It is not cosmetic.

| Class | Examples | Room topology? | Construction quantities? |
|---|---|---|---|
...
**FF&E never bounds a room and never affects a construction quantity.**

Every query that feeds room topology or QS must filter by class:
...
-- Room topology input
WHERE element_class = 'BUILDING' AND element_type = 'WALL'
```

A missed filter puts a sofa in a wall area. Index accordingly:

```text
idx_building_elements_project_class
  (project_id, element_class) WHERE deleted_model_version IS NULL
```

---

# 36.2 FF&E — Catalogue Assets and Placed Instances (ADR-019)

Two records, deliberately separated so a catalogue change never rewrites project data.
...
----------
id
organization_id nullable          NULL = global catalogue
code
name
category                          SEATING | TABLE | STORAGE | BED |
                                  APPLIANCE | SANITARY | LIGHTING | ...
manufacturer nullable
product_code nullable
nominal_width_mm
nominal_depth_mm
nominal_height_mm
thumbnail_object_id nullable      → storage_objects
model_object_id nullable          → storage_objects (GLB/GLTF)
default_properties_jsonb
properties_schema_version
tags_jsonb
...
------------------
id
organization_id                   denormalized from ffe_assets for
                                  direct tenant filtering and RLS
ffe_asset_id
version
nominal dimensions (as above)
model_object_id
default_finishes_jsonb
unit_cost NUMERIC nullable        catalogue list price
currency
effective_from
...
An FF&E instance **is a `building_elements` row** with `element_class = 'FFE'`.
Its placement geometry lives in `geometry_jsonb`:
...
```json
{
  ...
  "rotationDeg": 90,
  "scale": 1.0,
  "dimensionOverrideMm": null
}
```

and `ffe_asset_version_id` pins **which catalogue version** it was placed from.

> **Why version-pinned.** If an instance referenced the asset directly, a
> catalogue price or dimension update would silently change an existing project's
> geometry and an issued estimate. Pinning the version makes catalogue evolution
> safe.

## Binary assets never live in relational columns

GLB/GLTF models and textures go to object storage via `storage_objects`, exactly
like every other large artifact (§27, §49).

**A GLB is not financial truth.** FF&E price comes from `ffe_asset_versions.unit_cost`,
never from the mesh.

---

# 36.3 Finish Assignments (ADR-020)

A finish binds to an element **surface**, not the whole element — which is how a
feature wall in one room is expressed without splitting the wall.
...
organization_id
project_id
element_id                        → building_elements
surface                           INTERIOR_A | INTERIOR_B | EXTERIOR |
                                  TOP | BOTTOM | SOFFIT | ALL
room_id nullable                  which room this face serves
material_version_id               → material_versions
coverage                          FULL | PARTIAL
coverage_area_mm2 nullable        required when coverage = PARTIAL
...
```text
uq_finish_assignments_element_surface
  (element_id, surface) WHERE deleted_model_version IS NULL
```

## Three identity axes, three columns

```text
element_type_id       WHAT IT IS          on building_elements
assembly_version_id   WHAT IT'S BUILT OF  on building_elements
finish_assignments    WHAT IT LOOKS LIKE  separate table, surface-scoped
```

**Repainting a wall inserts a finish assignment. It never creates a new assembly
version.**

## Materials carry visual AND construction data

`materials` / `material_versions` (§51–§52) must carry both, because the renderer
and the cost engine read the same record:

```text
Visual (renderer)              Construction (QS / cost)
  base_colour                    unit_of_measure       m² | m | nr
  texture_object_id              coverage_rate         e.g. m² per litre
  normal_map_object_id           coats
  roughness · metallic           waste_profile_id
  opacity · texture_scale        labour_constant
                                 rate_reference
...
## Finish quantities are ruleset-governed

Whether a small opening is deducted from a painted wall area is decided by the
**QS ruleset** (ADR-009), never by the finish assignment and never by a domain
constant. Finish areas are measured quantities like any other.

---

# 36.4 Saved Views (presentation state)

Saved views are **presentation state, not geometry authority**.
...
project_id
name
view_kind                         ORBIT_3D | WALKTHROUGH | PLAN_2D
camera_jsonb                      position · target · projection · fov
visibility_jsonb                  visible levels · hidden categories ·
                                  isolated element ids
section_planes_jsonb nullable
model_version nullable            the version it was captured against
created_by_user_id
created_at
updated_at
```

A saved view **never mutates canonical geometry**. `model_version` is recorded so
a stale view can be flagged, not so it can override the model.

Walkthrough camera position, movement state and collision volumes are **runtime
renderer state** and are not persisted at all unless captured in a saved view.
...
## Element class filtering is mandatory (ADR-019)

A quantity run reads **construction fabric only**:

```sql
WHERE element_class = 'BUILDING'
  AND deleted_model_version IS NULL
```

FF&E is **excluded** from every construction quantity. It is costed separately
(§64.1). Rooms (`element_class = 'SPACE'`) contribute **finish** quantities —
floor, ceiling, skirting, wall finish — not fabric quantities.

A missed class filter puts furniture in a wall area. There is a mandatory test.
...
## FF&E is its own top-level section (ADR-019)

FF&E is priced as **product unit price × count**, not quantity × assembly rate.
It therefore forms a separate top-level BOQ section:

```text
boq_sections
  code = "FFE"    name = "Furniture, Fixtures & Equipment"
```
...
------------
...
include_ffe            boolean — default false
```

This lets a **construction-only estimate** be issued with or without furnishing,
which is the normal professional requirement — a contractor prices the building,
not the client's sofa.
...
project_id
model_version
quantity_run_id                 REQUIRED — pins the quantity lineage
boq_version_id
rate_book_version_id
cost_assumption_set_id          REQUIRED — pins overhead/profit/
cost_assumption_set_version     contingency/tax assumptions
qs_rule_set_version             pins the measurement ruleset (ADR-009)
estimate_level
currency
subtotal NUMERIC
waste_total NUMERIC
...
> **LOCKED — ADR-015.** All six lineage inputs are **required and immutable** once
> the estimate is issued: model version, quantity run, BOQ version, rate book
> version, cost engine version, and assumption set version (plus the QS ruleset
> version that produced the quantities).
>
> *(Corrected 2026-09-12: `quantity_run_id` and the assumption-set reference were
> missing from this table, so an estimate could not actually be reproduced —
> changing project assumptions would silently change a historical figure.
> Independent review found this contradicted ADR-015.)*
>
> **On "byte-identical".** Recalculation guarantees **numeric equivalence** from the
> pinned tuple. A regenerated PDF is *not* guaranteed byte-identical, because that
> also depends on template, renderer, fonts, locale and timezone. **Issued reports
> are therefore preserved as immutable, checksummed artifacts in object storage**
...
plan_versions
plan_prices
feature_definitions        runtime-editable catalogue (ADR-022)
plan_entitlements
plan_entitlement_overrides per-org, immediate (ADR-022)
billing_subscriptions      ← carries term_model / term_ends_at /
                             auto_renew (ADR-021)
billing_periods
...
## Two term models (ADR-021)

`billing_subscriptions` supports **RECURRING** and **FIXED_TERM**:
...
A fixed term ends with **no cancellation event** and moves the associated project
to `archived` (§23) with data preserved. Column detail lives in
`24_Billing_AI_Credits_and_Usage.md` §18.
...
--------------------------
id · organization_id · feature_code · value
reason                    REQUIRED — appears in the audit trail
effective_from · effective_to (nullable = until revoked)
created_by_admin_id · created_at
```

Resolution order (highest first): enterprise override → **organization override**
→ add-on/promotion → plan version entitlement → `default_value`.

Indexes:

```text
uq_feature_definitions_code            UNIQUE (code)
idx_plan_entitlement_overrides_org
  (organization_id, feature_code) WHERE effective_to IS NULL

idx_plan_entitlement_overrides_active
  (feature_code) WHERE effective_to IS NULL
```

**Classification:** `feature_definitions` and `plan_entitlement_overrides` are
**authoritative** (§206) — an override is a commercial commitment to a customer
and cannot be recomputed. Both survive project deletion; both are
organization-scoped except `feature_definitions`, which is global platform data.
...
Buildora AI stores business references:
...
## `archived` is NOT part of this flow (ADR-021)

```text
draft → active → archived → deleting → deleted
                    ▲          ▲
        term ends HERE    explicit deletion request only
```

**An expired paid term never triggers deletion.** Archive preserves everything;
only an explicit request (or a retention policy) enters `deleting`.

## Deletion order — project-scoped tables

Delete derived data first, then authoritative, so a failed run never leaves
quantities pointing at absent elements:

```text
1. rebuildable     model_artifacts · document_chunks · embeddings
                   retrieval_runs · saved_views
2. derived         cost_estimates → boq_* → quantity_* (reverse lineage order)
3. authoritative   finish_assignments · element_relationships
                   building_elements (all classes incl. FFE)
                   model_change_items · model_commands · model_snapshots
                   model_versions · levels · buildings · sites
4. project row
5. object storage  referenced storage_objects, after DB rows are gone
```

**Not deleted with a project:**

```text
ffe_assets / ffe_asset_versions    organization-level catalogue, shared
materials / assemblies             organization-level libraries
rate_books                         organization-level
suppliers                          organization-level
feature_definitions                GLOBAL platform data
plan_entitlement_overrides         organization-level commercial record
audit_events · billing records     retained per legal/retention policy
```

**Post-MVP project-scoped tables** join step 3 (authoritative) when built:

```text
schedule_versions · schedule_tasks · schedule_dependencies
rfqs · rfq_items · supplier_quotes · purchase_orders · deliveries
actual_cost_entries · change_orders
```

Deleting a project must never orphan a catalogue another project still uses.
...
# 133.1 Element Class, FF&E and Finish Indexes

```text
idx_building_elements_project_class
  (project_id, element_class) WHERE deleted_model_version IS NULL

idx_building_elements_project_level_class
  (project_id, level_id, element_class) WHERE deleted_model_version IS NULL

idx_finish_assignments_element
  (element_id) WHERE deleted_model_version IS NULL

idx_finish_assignments_project_room
  (project_id, room_id) WHERE deleted_model_version IS NULL

uq_finish_assignments_element_surface
  UNIQUE (element_id, surface) WHERE deleted_model_version IS NULL

idx_ffe_assets_org_category
  (organization_id, category) WHERE status = 'ACTIVE'

idx_ffe_assets_global_category
  (category) WHERE organization_id IS NULL AND status = 'ACTIVE'

idx_ffe_asset_versions_asset
  (ffe_asset_id, version)

idx_building_elements_ffe_asset
  (ffe_asset_version_id) WHERE element_class = 'FFE'
                           AND deleted_model_version IS NULL

idx_saved_views_project
  (project_id, view_kind)
```

The class-filtered partial indexes matter because **every** QS and topology query
filters on `element_class`; without them those become sequential scans as
furniture counts grow.
...
## Retention by table category

| Category | Tables | Retention |
|---|---|---|
| **Authoritative project** | `building_elements` · `element_relationships` · `finish_assignments` · `model_versions` · `model_commands` · `model_change_items` · `levels` · `buildings` · `sites` | While the project exists — **including archived** |
| **Authoritative org library** | `materials` · `assemblies` · `rate_books` · `ffe_assets` · `qs_rule_sets` · `cost_assumption_sets` | While the organization exists, or while referenced |
| **Issued commercial record** | `cost_estimates` · `boq_versions` · `report_artifacts` | Retained as issued — **never regenerated in place** (ADR-015) |
| **Recomputable commercial** | `quantity_runs` · `quantity_items` · `boq_items` | While their estimate is retained; recomputable from pinned lineage |
| **Legal / financial** | `credit_ledger_entries` · `invoice_records` · `payment_records` · `billing_*` | Accounting and legal period — **survives project deletion** |
| **Audit** | `audit_events` · `inbox_events` · `outbox_events` | Policy/legal period; survives project deletion; time-partitioned |
| **Rebuildable** | `model_artifacts` · `document_chunks` · `document_chunk_embeddings` · `retrieval_runs` | Discardable; regenerate on demand |
| **Ephemeral** | `processing_jobs` · `idempotency_records` · presence | Short, bounded |
| **Presentation** | `saved_views` · `user_preferences` | While the project/user exists |
| **Platform catalogue** | `feature_definitions` | Indefinite; retire with `is_active = false`, never delete — historical entitlement snapshots reference the code |
| **Commercial commitment** | `plan_entitlement_overrides` | Retained with billing records; an expired override is **history**, not garbage |
| **Post-MVP modules** | `schedule_*` · `suppliers` · `rfqs` · `purchase_orders` · `actual_cost_entries` · `change_orders` | While the project exists, including archived |

> **`finish_assignments` sits with authoritative project data, not derived.** A
> user's colour choice cannot be recomputed from geometry — treating it as
> rebuildable would erase every finish on the next rebuild.

## Archived projects (ADR-021)

An archived project is **retained in full** — it is not a deletion candidate.

```text
archived project data          retained indefinitely while the account exists
model_artifacts (GLB, etc.)    lifecycle → infrequent-access storage class
                               after a defined idle period
document_chunks / embeddings   may be dropped and rebuilt on reactivation
finish_assignments             retained — authoritative user intent,
                               NOT reconstructible from geometry
saved_views                    retained (cheap, user intent)
```

**Never delete an archived project's authoritative data as a storage
optimisation.** Reactivation depends on it, and reactivation is the revenue case
that justifies archive mode.

Only an explicit deletion request, account closure, or a stated retention policy
moves an archived project to `deleting`.

## Catalogue assets

```text
ffe_assets / ffe_asset_versions    retained while ANY instance references them
                                   — including instances in archived projects
```

A catalogue version referenced by a placed instance is never deleted; deprecate
it (`status = DEPRECATED`) so it stops appearing in the picker while existing
placements stay resolvable.
...
**Every table must be classified.** The class determines backup priority, deletion
behaviour, whether it may be rebuilt, and whether losing it is a data-loss incident.

## Authoritative — loss is unrecoverable

```text
users · organizations · organization_members · projects
building_elements            (all classes: BUILDING, SPACE, FFE)
element_relationships
finish_assignments           surface finishes — NOT reconstructible from geometry
model_versions · model_commands · model_change_items
materials · material_versions · assemblies · assembly_versions
rate_books · rate_book_versions · cost_rates
qs_rule_sets · cost_assumption_sets
ffe_assets · ffe_asset_versions        catalogue — tenant or global
feature_definitions                    platform feature catalogue (ADR-022)
plan_entitlement_overrides             commercial commitment to a customer
documents · document_revisions · storage_objects
credit_ledger_entries · credit_grants · credit_reservations
billing_subscriptions · invoice_records · payment_records
audit_events · inbox_events · outbox_events
saved_views                  user intent; cheap but not reconstructible

POST-MVP MODULES (same discipline applies when built)
schedule_versions · schedule_tasks · schedule_dependencies
suppliers · rfqs · supplier_quotes · purchase_orders · deliveries
actual_cost_entries · change_orders
classification_systems · project_templates
```

> **`plan_entitlement_overrides` is authoritative, not derived.** An override is
> a commercial commitment to a customer — it cannot be recomputed from a plan.
> Losing it silently revokes access someone was promised.

> **`feature_definitions` is global platform data**, not tenant-scoped. It is the
> one table in this list without an `organization_id`.

> **`finish_assignments` is authoritative, not derived.** A user's colour choice
> cannot be recomputed from geometry. Classifying it as derived would make a
> rebuild silently erase every finish in every project.

## Derived but persisted — reproducible, yet immutable once issued

```text
quantity_runs · quantity_items · quantity_item_sources
boq_versions · boq_sections · boq_items · boq_item_quantity_links
...
These are recomputable **from their pinned lineage** (ADR-015) — but an **issued**
estimate or report is never regenerated in place. It is retained as the record of
what the customer was given.

## Rebuildable — safe to discard and regenerate

```text
model_artifacts              GLB · Fragments · thumbnails · 2D previews
document_chunks · document_chunk_embeddings
retrieval_runs · retrieval_results
dashboard aggregates and read models
```

FF&E **mesh assets** (`storage_objects` referenced by `ffe_asset_versions`) are
rebuildable only if the source file is retained elsewhere; treat an uploaded
customer asset as **authoritative**, and a re-downloadable global catalogue asset
as rebuildable.

Mark the class in table documentation and in the backup plan.
...
If Buildora AI offers API access:
...
erDiagram
    ...
    BUILDING_ELEMENTS ||--o{ ELEMENT_RELATIONSHIPS : relates
    BUILDING_ELEMENTS ||--o{ FINISH_ASSIGNMENTS : finished_by
    MATERIAL_VERSIONS ||--o{ FINISH_ASSIGNMENTS : specifies
    FFE_ASSET_VERSIONS ||--o{ BUILDING_ELEMENTS : placed_as
    FFE_ASSETS ||--o{ FFE_ASSET_VERSIONS : versions
    PROJECTS ||--o{ SAVED_VIEWS : has
```

`BUILDING_ELEMENTS` carries `element_class` — `BUILDING`, `SPACE` or `FFE`
(ADR-019). One table, three classes; the class governs which subsystems read it.

An `FFE` element references `FFE_ASSET_VERSIONS`, never `FFE_ASSETS` directly,
so catalogue evolution cannot alter a placed instance.
...
erDiagram
    ...
    QUANTITY_RUNS ||--o{ COST_ESTIMATES : pins
    COST_ASSUMPTION_SETS ||--o{ COST_ESTIMATES : pins
    QS_RULE_SETS ||--o{ QUANTITY_RUNS : governs

    FINISH_ASSIGNMENTS ||--o{ QUANTITY_ITEMS : measured_as
    FFE_ASSET_VERSIONS ||--o{ BOQ_ITEMS : priced_in
```

## Two quantity paths, one engine

```text
element_class = BUILDING   → fabric quantities      quantity × assembly rate
element_class = SPACE      → finish quantities      via finish_assignments
element_class = FFE        → FF&E cost section      unit price × count
                             (boq_versions.include_ffe)
```

**FF&E is excluded from fabric quantities entirely** (ADR-019). It is priced from
`ffe_asset_versions.unit_cost`, not from an assembly rate — which is why it needs
its own BOQ section rather than a line among the blockwork.

A cost estimate pins **all six** lineage inputs (ADR-015): model version,
quantity run, BOQ version, rate book version, assumption set and engine version —
plus the QS ruleset that produced the quantities.
...
> **Includes `element_class` and `ffe_asset_version_id` on `building_elements`
> from the first migration** (ADR-019), even though FF&E itself is not built until
> Phase 3b. The column is cheap now and a full-table reclassification later.
...
# 244.1 Phase 3b — Interior, Finishes & FF&E

Follows the commercial engine; **not MVP** (ADR-019, ADR-020).

```text
finish_assignments
materials / material_versions extended with construction properties
  (unit_of_measure · coverage_rate · coats · waste · labour · rate ref)
ffe_assets
ffe_asset_versions
saved_views
boq_versions.include_ffe
BOQ "FFE" section seed
```

> **`element_class` and `ffe_asset_version_id` ship in Phase 2 with
> `building_elements`, not here.** Adding a class column after elements exist means
> reclassifying every row and re-auditing every quantity query — which is exactly
> the retrofit ADR-019 exists to prevent.
...
Org A cannot read Org B finish assignments
Org A cannot read Org B FF&E placements
Org A cannot read Org B saved views
Org A cannot read Org B private FF&E catalogue
Org A cannot read Org B rate books, materials or assemblies
Org A cannot read an ARCHIVED Org B project
```

These are release-blocking tests.

> **Archived is not less protected.** An archived project is read-only to its
> *owner* — it is not readable by anyone else. Archive changes capability, never
> tenancy.

> **Global catalogue is the one deliberate exception.** `ffe_assets` and
> `materials` with `organization_id IS NULL` are readable by every tenant by
> design. Test that the *inverse* holds: a row **with** an `organization_id` is
> never visible to another organization.
...
## Class and lineage integrity (ADR-015, ADR-019, ADR-020)

These catch the failure modes that are silent and expensive:

```text
-- No FF&E in a construction quantity  (ADR-019)
quantity_item_sources → building_elements
  WHERE element_class <> 'BUILDING'          → must return 0 rows

-- Finish assignments reference a live element and a real material
finish_assignments
  WHERE element_id has no live building_elements row
     OR material_version_id is absent          → must return 0 rows

-- One finish per element surface
finish_assignments
  GROUP BY element_id, surface
  HAVING count(*) > 1 (live rows only)         → must return 0 rows

-- Every issued cost estimate pins its full lineage  (ADR-015)
cost_estimates
  WHERE quantity_run_id IS NULL
     OR cost_assumption_set_id IS NULL
     OR rate_book_version_id IS NULL
     OR engine_version IS NULL                 → must return 0 rows

-- FF&E instances resolve to a catalogue VERSION, not a bare asset
building_elements
  WHERE element_class = 'FFE'
    AND ffe_asset_version_id IS NULL           → must return 0 rows

-- Catalogue versions referenced by live instances still exist
building_elements (FFE) → ffe_asset_versions   → no dangling references

-- Archived projects retain their data  (ADR-021)
projects WHERE status = 'archived'
  AND no building_elements rows                → investigate: possible data loss

-- Catalogue assets referenced by live placements are never deleted
ffe_assets referenced by any live FFE element
  WHERE row absent                             → must return 0 rows

-- Saved views reference a project that still exists
saved_views WHERE project_id has no project    → must return 0 rows

-- Denormalized tenant key stays consistent
ffe_asset_versions.organization_id
  <> ffe_assets.organization_id                → must return 0 rows
```

Do not run expensive full checks on every request. Run them nightly and alert on
any non-zero result — each of these represents corruption that no user would
notice until an estimate is already wrong.
...
This is one of Buildora AI's most important professional differentiators.

---

# 261.1 Data Lineage — Interior Finish and FF&E

## Finish (ADR-020)

```text
Cost estimate line "Emulsion, 2 coats, warm grey"
    ↑ rate                rate_book_versions → cost_rates
    ↑ quantity            quantity_items (measurement_type = AREA)
    ↑ measured from       finish_assignments (element_id + surface)
    ↑ area derived from   building_elements (wall) + rooms (interior faces)
    ↑ ruleset             qs_rule_sets (deduction thresholds)
    ↑ material            material_versions (coverage rate, coats, waste)
    ↑ created by          model_commands (SET_ELEMENT_FINISH)
    ↑ at                  model_versions
    ↑ by                  source_kind MANUAL | AI + source_ref
```

## FF&E (ADR-019)

```text
FF&E cost line "3-seat sofa × 1"
    ↑ unit price          ffe_asset_versions.unit_cost
    ↑ product             ffe_assets (manufacturer, product_code)
    ↑ placed instance     building_elements (element_class = FFE)
    ↑ created by          model_commands (PLACE_FURNITURE)
    ↑ at                  model_versions
    ↑ by                  source_kind MANUAL | AI + source_ref (AI job id)
```

**Both chains answer the two questions a professional actually asks:**
*"why is this number what it is?"* and *"who or what put this here?"* — the
second is why `source_kind` is mandatory on every element from day one.
...
## Additional checks for model, interior and commercial features

- [ ] Does every element query that feeds topology or QS filter on
      `element_class`? (ADR-019 — a missed filter puts furniture in a wall area)
- [ ] Is a new element type assigned the correct class — `BUILDING`, `SPACE` or `FFE`?
- [ ] Does a finish change create a `finish_assignments` row rather than a new
      assembly version? (ADR-020)
- [ ] Does a new material carry **construction** properties (unit, coverage,
      coats, waste, labour, rate reference) and not only visual ones?
- [ ] Does an FF&E instance reference an asset **version**, never a bare asset?
- [ ] Does a new derived-commercial table pin its full lineage? (ADR-015)
- [ ] Is the table classified authoritative / derived / rebuildable? (§206)
- [ ] Is it in the project deletion order, or explicitly excluded as
      organization-level? (§124)
- [ ] Does it carry `organization_id`, with a tenant-isolation test? (ADR-016)
- [ ] Does an archived project behave read-only without losing data? (ADR-021)

---

# 267. Final Architecture

The Buildora AI database should behave like this:
...
# 267.1 Completeness Rules for This Document

A table is **not finished** when its columns are written. It is finished when all
seven of these are true:

```text
1. classified                authoritative | derived | rebuildable   (§206)
2. tenant-scoped             organization_id + isolation test        (ADR-016)
3. indexed                   for its real query patterns             (§132–§138)
4. in the deletion order     or explicitly excluded as org-level     (§124)
5. in retention policy       especially for archived projects        (§181)
6. lineage-complete          if it is derived commercial data        (ADR-015)
7. integrity-checked         a nightly query that proves it correct  (§255)
```

Any new table added after this document is frozen must satisfy all seven before
its migration is written.

---

# 268. Final Database Principles

Buildora AI must preserve these rules:
...
```text
1. Read 15_Database_and_Data_Architecture.md.
...
The database is not merely where Buildora AI saves application objects.
...
what Buildora AI paid the AI provider
