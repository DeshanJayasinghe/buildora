# Buildora AI — Sample Frontend CRUD Reference

> **File name:** `Buildora_AI_Sample_Frontend_CRUD.md`
>
> **Purpose:** Canonical sample frontend CRUD implementation for Buildora AI.
>
> **Example feature:** `Projects`
>
> **Stack:** Next.js App Router + React + TypeScript + TanStack Query + React Hook Form + Zod + Buildora AI Design System
>
> This sample follows the Buildora AI frontend standards:
>
...
>
> **Scope — read before copying.**
> This is the pattern for **ordinary SaaS CRUD**: projects, materials, suppliers,
> documents, rate libraries, report templates.
>
> It is **not** the pattern for the Building Model. A wall is not an admin record.
> Editing geometry goes through `packages/model-session` and the model-command API
> (ADR-001, ADR-002, ADR-008) — see §48 and the backend sample's model-command
> section. Do not build an "Edit Wall Form" that issues `PATCH /walls/:id`.
>
> IDs follow ADR-004: `uuid` primary keys, with `public_id` for display only.
...
If Buildora AI uses same-origin proxying, this helper can simply return:
        ...
        new QueryClient({
          defaultOptions: {
            queries: {
              ...
              retry:
                (failureCount, error) => {
                  // `status` is a getter on ApiError.prototype,
                  // so `"status" in error` is FALSE. Use instanceof.
                  if (
                    ...
                    // Never retry client errors except throttling.
                    if (
                      status >= 400 &&
                      status < 500 &&
                      status !== 429
...
> **A real bug worth naming.** An earlier version of this sample tested
> `"status" in error`. That is **false** for `ApiError`, because `status` is a
> getter defined on the prototype rather than an own property — so every 401, 403,
> 404 and 422 was retried twice. Use `instanceof ApiError`.

401 is handled centrally (see §36), not by retrying.
  ...
  if (
    ...
    return (
      <EmptyState
        ...
        description="Create your first Buildora AI project."
      />
    );
  }
...
The real Buildora AI design-system field components should reduce repetitive label/error markup.
    ...
    >
      ...
      >
        <div>
          ...
          <p>
            Manage your Buildora AI projects.
          </p>
        </div>
  ...
  const handleDelete =
    async () => {
      try {
        ...
        // Dialog stays open; mutation state renders the error.
      }
    };
...
- The dialog is **controlled** (`open` / `onOpenChange`). An uncontrolled `Dialog`
  has no way to close itself after a successful mutation.
- On failure the dialog **stays open** so the error is visible and the action is
  retryable.
- An explicit **Cancel** gives predictable focus return and a non-destructive exit.

Production Buildora AI may prefer archive/retention rather than immediate permanent deletion.
      ...
      () => {
        render(
          <ProjectCard
            project={{
              id:
                "019a3f2c-8d41-7c3e-9b02-4f7a1c6e5d88",

              publicId:
                "PRJ-8K4D2A",

              organizationId:
                "019a3f2c-1111-7000-8000-000000000001",
        ...
        expect(
          ...
          "href",
          "/projects/019a3f2c-8d41-7c3e-9b02-4f7a1c6e5d88",
        );
      },
    );
  },
...
This pattern should be used consistently throughout normal Buildora AI SaaS CRUD features.
